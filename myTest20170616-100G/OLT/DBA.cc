...
//    }
    ...
    if (fourChtime > 0) // means use four channel
        avgChannaltime = (1 * oneChtime + 2 * twoChtime + 3 * threeChtime + 4 * fourChtime + active_ch * (simTime() - lastChangeChtime)) / 10;
    else if (threeChtime > 0)  // means use three channel
        avgChannaltime = (1 * oneChtime + 2 * twoChtime + 3 * threeChtime + active_ch * (simTime() - lastChangeChtime)) / 10;
    else if (twoChtime > 0) // means use two channel
        avgChannaltime = (1 * oneChtime + 2 * twoChtime + active_ch * (simTime() - lastChangeChtime)) / 10;

    if ( lastChangeChtime < 1 ) avgChannaltime = active_ch * (simTime() - lastChangeChtime) / 10;
...
//    cout << "onu[" << idx << "] " << curMode << "   reqLength=" << reqLength<< " grant=" << grantUpLen[idx]<< endl;}
    ...
    */
    if (idx+1 == onuSize ) { // last onu
        ...
        active_ch = ceil( (((double) cycleAllPacketSize) * 8.0)/ 50000000 )  ; // onu * 0.002 cycle time
